...
const contractAddress = '0x84dCCAfbdd900e1F9365b94aeebA766A6966a3C1';
const contractABI = [
  {
    "inputs": [],
...
const getAllCandidates = async function(){
  ...
  for(let i = 0; i < candidates.length; i++) {
    ...
    candidate.innerHTML = `
      <td>${parseInt(candidates[i][0])}</td>
      <td>Candidate ${candidates[i][1]}</td>
    `
...
const getAccount = async function() {
  const ethAccounts = await provider.send("eth_requestAccounts", []).then(() => {
    provider.listAccounts((accounts) => {
      signer = provider.getSigner(accounts[0]);
      contract = new ethers.Contract(contractAddress, contractABI, signer)
    })
  })
