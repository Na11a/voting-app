...
contract Voting {
  ...
  function startElection(string[] memory _candidates, uint256 _votingDuration) public {
    require(!electionStarted, "Election is currently ongoing");
    delete candidates;

    for(uint64 i = 0; i < _candidates.length; i++){
      candidates.push(
  ...
  function addCandidate(string memory _name) public onlyOnwer electionOngoing {
  ...
  function voteTo(uint256 _id) public electionOngoing {
    require(checkElectionPeriod(), "Election Period has ended");
    require(!voterStatus(msg.sender), "You already voted. You can only votte once");
  ...
  function resetAllVoterStatus() public onlyOnwer {
